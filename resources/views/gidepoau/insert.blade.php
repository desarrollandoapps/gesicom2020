@extends('layouts.app', ['activePage' => 'gidepoau', 'titlePage' => __('gidepoau')])
        ...
        <div class="container-fluid">
                                            ...
                                            @foreach ($autores as $item)
                                                <tr>
                                                    ...
                                                    <td class="td-actions text-right">
                                                        <form action="{{route('gidepoau.destroy', $item->idDetalle)}}" method="POST" class="d-inline">
                                                            @csrf
                                                            @method('DELETE')
                                                            <input type="hidden" name="dpponinv" value="{{$ponencia->id}}">
                                                        </form>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <h4 class="card-title">{{__('gidepoau')}} </h4>
                        </div>
                        <div class="card-body">
                            @if ($errors->any())
                            ...
                            <hr>
                            <form action="{{route('gidepoau.store')}}" class="needs-validation" method="POST" id="formAdd" novalidate>
                                @csrf
                                @method('POST')
                                <input type="hidden" name="dpponinv" value="{{$ponencia->id}}">
...
@section('scripts')
    <script>
        ...
        $('#ligruinv').change(function(event){
            const aprendiz = "<?php echo $esAprendiz ?>";
            if(aprendiz == 1)
            {
                $.get("../integrantesemillero/" + event.target.value, function(response, investigadores){
                    $('#dpinvest').empty();
                    $('#dpinvest').append("<option value=''>Seleccione...</option>");
                    for(i = 0; i < response.length; i++)
                    {
                        $('#dpinvest').append("<option value='" + response[i].id + "'>" + response[i].sinombre + "</option>");
                    }
                });
            }
            else {
        ...
        function desasociar(id) {
            swal({
                ...
                if (e.value === true) {
                    var idunico = id;
                    var rutamala = "{{ route('borrandoAutorPo', "reempl") }}";
